...
/**
   ...
   @type {object}
   @module crosscloud
   @property {object} yydefaultClient is...
 */
var crosscloud = exports;

crosscloud.version = '!!VERSION!!';

// The new API with v0.2.0

/**
   Use this as the Client() object, mostly.

   @memberof crosscloud
   @type {object}
   @namespace crosscloud
   @property {method} xxdefaultClient is...
 */
crosscloud.defaultClient = new Client();

Object.defineProperty(crosscloud, "focusURL", {
	value: podlogin.focusPageLocation,
	writable: false
});
...
crosscloud.globalPod = null;


crosscloud.suggestProvider = function (url) {
	console.log('suggested provider:', url);
	// ignore for now
};
...
// obsolete?  use pod.focusPageLocation instead
// crosscloud.onFocusPage = podlogin.onFocusPage;
// crosscloud.getFocusPage = podlogin.getFocusPage;

crosscloud.connect = function (options) {
	var pod;
	if (options && options.podURL) {
		pod = new crosscloud.PodClient();
		console.log("connecting to ", options.podURL);
		pod.connect(options.podURL);
		return pod;
	}
    if (crosscloud.globalPod) {
        return crosscloud.globalPod;
    }
    pod = new crosscloud.PodClient();
    ...
    crosscloud.globalPod = pod; 
    return pod;
};

crosscloud.PodClient = function PodClient(){
    if ( !(this instanceof PodClient) ) {
        throw new Error("Constructor called as a function. Must use 'new'");
    }

    var pod = this;
	// fortunately, podlogin.focusPageLocation is set at load time, before
	// this code could ever be run
	Object.defineProperty(pod, "focusPageLocation", {
		value: podlogin.focusPageLocation,
		writable: false
	});
...
var pod = crosscloud.PodClient.prototype;

// Just pass these all through to podlogin
// 
// For now, we don't want app developers to be away of podlogin
//
...
pod.onLogout = function (f) {
    podlogin.onLogout(f);
};
pod.requireLogin = function (msg) {
    podlogin.requireLogin(msg);
};
pod.forceLogout = function () {
    podlogin.forceLogout();
};

// SEPARATELY, we need an onConnect, onDisconnect, I think.
// which might be triggered by network events, or by login
// activity.
