...
function fail(s) {
	console.log('**ERROR**', s);
	// for some reason thrown errors are being ignored...  :-(
	throw new Error(s);
}
...
var WebCircuit = function (addr) {
    ...
    if (!(this instanceof WebCircuit)) fail("Use 'new' Please");
    ...
    wc.send = function (op, args, onPush) {
        if ( typeof op !== typeof "" ) fail("Bad Parameter");
        if ( typeof args !== typeof {} ) fail("Bad Parameter");
    ...
    wc.connect = function (addr) {
        return new Promise(function(resolve, reject) {
			...
			if (wc.ws !== null) {
				reject(new Error("already connected; we don't handle login/logout yet"))
				return
			}
